...
class SnapbotGymClass():
    ...
    def step(self,a,max_time=np.inf):
        ...
        self.target_x, self.target_y = 2.0, -3.0
        # 현재 y 위치 및 이동량
        x_prev, x_curr = p_torso_prev[0], p_torso_curr[0]
        y_prev, y_curr = p_torso_prev[1], p_torso_curr[1]
        x_diff = x_curr - x_prev
        y_diff = y_curr - y_prev
        x_toward = np.sign(self.target_x - x_prev) == np.sign(x_diff)
        y_toward = np.sign(self.target_y - y_prev) == np.sign(y_diff)
        x_reached = abs(x_curr - self.target_x) < 0.3
        y_reached = abs(y_curr - self.target_y) < 0.3

        target_pos = np.array([self.target_x, self.target_y])
        current_pos = np.array([x_curr, y_curr])
        ...
        at_target = x_reached and y_reached

        if not at_target:
            # 목표 쪽으로 이동한 경우 보상
            r_sidewalk = 0.0
            if x_toward:
                r_sidewalk += abs(x_diff) / self.dt
            else:
                r_sidewalk -= abs(x_diff) / self.dt

            if y_toward:
                r_sidewalk += abs(y_diff) / self.dt
            else:
                r_sidewalk -= abs(y_diff) / self.dt
        else:
            r_sidewalk = 20.0  # 도착 보너스

            # 도착 이후 이동하면 감점
            drift = abs(x_diff) + abs(y_diff)
            if drift > 1e-3:  # 아주 미세한 움직임 허용
                r_sidewalk -= drift * 100.0
        
        # 가까워질수록 속도가 작아지도록 유도하는 보상
        if distance_to_target < 0.5:
            r_slow_down = -speed * 2.0  # 속도가 크면 감점
        else:
            r_slow_down = 0.0
        ...
        target_direction = np.array([
            self.target_x - x_curr,
            self.target_y - y_curr,
            0.0
        ])

        target_direction = target_direction / (np.linalg.norm(target_direction) + 1e-6)

        target_direction_perp = np.array([
            -target_direction[1],
            target_direction[0],
            0.0
        ])

        heading_vec = R_torso_curr[:, 0]  # 로봇의 정면 방향 벡터
        r_heading = 0.01 * np.dot(heading_vec, target_direction_perp)

        if r_heading < 0.0:
            r_heading *= 100.0  # 잘못된 방향은 강하게 패널티
        else:
            r_heading * 2.0
            
        
        # Compute reward
        r = np.array(r_sidewalk + r_collision + r_survive + r_heading + r_slow_down)
        ...
        # Other information
        info = {'yaw_torso_deg_prev':yaw_torso_deg_prev,
                'yaw_torso_deg_curr':yaw_torso_deg_curr,
                'x_diff':x_diff,
                'y_diff':y_diff,
                'SELF_COLLISION':SELF_COLLISION,
                'r_sidewalk':r_sidewalk,
