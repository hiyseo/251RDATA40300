...
class SnapbotGymClass():
    ...
    def step(self,a,max_time=np.inf):
        ...
        x_curr, x_prev = p_torso_curr[0], p_torso_prev[0]
        y_curr, y_prev = p_torso_curr[1], p_torso_prev[1]
        z_curr, z_prev = p_torso_curr[2], p_torso_prev[2]

        z_velocity = (z_curr - z_prev) / self.dt
        x_diff = x_curr - x_prev # x-directional displacementy
        y_diff = y_curr - y_prev # y-directional displacement

        r_velocity = max(z_velocity, 0.0) * 10.0  # 음수면 무시
        r_drift = -(x_diff + y_diff) * 10.0

        r_vertical = r_velocity + r_drift
        ...
        # Compute reward
        r = np.array(r_vertical + r_collision + r_survive)
